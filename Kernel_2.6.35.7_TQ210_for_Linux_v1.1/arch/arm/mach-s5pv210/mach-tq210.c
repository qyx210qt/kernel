...
static struct s3cfb_lcd tq210_lcd_param = {
	.width	= 800,
	.height	= 480,
	.bpp	= 16,
	.freq	= 70,
...
static void tq210_lcd_cfg_gpio(struct platform_device *pdev)
{
	int i;
...
static int tq210_lcd_backlight_on(struct platform_device *pdev)
{
	int err;
...
static int tq210_lcd_backlight_off(struct platform_device *pdev, int onoff)
{
	int err;
...
static int tq210_lcd_reset_lcd(struct platform_device *pdev)
{
	return 0;
}

static struct s3c_platform_fb tq210_fb_data __initdata = {
	.hw_ver	= 0x62,
	.nr_wins = 5,
	...
	.lcd = &tq210_lcd_param,
#endif
	.cfg_gpio	= tq210_lcd_cfg_gpio,
	.backlight_on	= tq210_lcd_backlight_on,
	.backlight_onoff    = tq210_lcd_backlight_off,
	.reset_lcd	= tq210_lcd_reset_lcd,
};
#endif
...
#if defined(CONFIG_FB_S3C_LTE480WV)||defined(CONFIG_FB_S3C_LCD800480)||defined(CONFIG_FB_S3C_LCD800600)||defined(CONFIG_FB_S3C_VGA1024768)||defined(CONFIG_FB_S3C_LCD640480)||defined(CONFIG_FB_S3C_LCD480272)||defined(CONFIG_FB_AT070TN92) ||defined (CONFIG_FB_S3C_VGA640480) || defined(CONFIG_FB_AUTO_SCAN_TQ_LCD)
	s3cfb_set_platdata(&tq210_fb_data);
#endif
#ifdef CONFIG_TQ210_VGA
...
//	printk("otg_clk = %d \n",clk_get_rate(otg_clk));
	...
	__raw_writel(__raw_readl(S3C_USBOTG_RSTCON) & ~(0x1<<4) & ~(0x1<<3),
			S3C_USBOTG_RSTCON);
	gpio_request(S5PV210_GPD0(2), "3G-POWER");
	s3c_gpio_cfgpin(S5PV210_GPD0(2), S3C_GPIO_SFN(1));//3G power up
...
void usb_host_phy_off(void)
{
	...
	__raw_writel(__raw_readl(S5P_USB_PHY_CONTROL) & ~(1<<1),
			S5P_USB_PHY_CONTROL);
	gpio_request(S5PV210_GPD0(2), "3G-POWER");
	s3c_gpio_cfgpin(S5PV210_GPD0(2), S3C_GPIO_SFN(1));//3G power down
